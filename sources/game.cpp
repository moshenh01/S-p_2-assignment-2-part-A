...
void  Game :: playWar(vector<Card> pile1,vector<Card> pile2,Player& p1,Player& p2)// check if its ok*****
{
   if (p1.getHasCards() && p2.getHasCards()){
        ...
        // Determine winner of the turn
        if (val1 > val2) {
            std::string docTurn = p1.getName() + " played " + p1Card.strValue() + " of " + p1Card.getSuitString(p1Card.getSuit()) + " ,"
                            + p2.getName() + " played " + p2Card.strValue() + " of " + p2Card.getSuitString(p2Card.getSuit()) + " "
                            + p1.getName() + " Wins! \n";
            this->log.back() += docTurn;
            

            p1.addTakenCard(p1Card);
            p1.addTakenCard(p2Card);
            player1.addNumOfWins();
            ...
            p2.addTakenCard(p1Card);
            p2.addTakenCard(p2Card);
            player2.addNumOfWins();
            ...
            if(p1.getHasCards() && p2.getHasCards()){
                ...
                pile1.push_back(p1Card);
                pile2.push_back(p2Card);
                pile1.push_back(p1.getTopCard());
                pile2.push_back(p2.getTopCard());
                playWar(pile1,pile2, p1, p2);
            }
            else{//return cards to players
                p1.addTakenCard(p1Card);
                p1.addTakenCard(p2Card);
